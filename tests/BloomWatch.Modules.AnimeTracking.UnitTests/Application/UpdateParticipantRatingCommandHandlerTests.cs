...
public sealed class UpdateParticipantRatingCommandHandlerTests
{
    ...
    private readonly UpdateParticipantRatingCommandHandler _handler;

    private readonly Guid _watchSpaceId = Guid.NewGuid();
    private readonly Guid _userId = Guid.NewGuid();

    public UpdateParticipantRatingCommandHandlerTests()
    {
        _handler = new UpdateParticipantRatingCommandHandler(_membershipChecker, _repository);
    }
    ...
    public async Task HandleAsync_HappyPath_ReturnsUpdatedParticipantDetail()
    {
        ...
        var command = new UpdateParticipantRatingCommand(
            _watchSpaceId, anime.Id.Value, _userId,
            RatingScore: 8.5m,
            RatingNotes: "Excellent pacing",
            UpdateNotes: true);
        ...
        result.RatingScore.Should().Be(8.5m);
        result.RatingNotes.Should().Be("Excellent pacing");
