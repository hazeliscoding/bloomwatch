...
The system SHALL allow a member with the `Owner` role to invite another user by their registered email address. The system SHALL create an `Invitation` record with a unique opaque token and an expiry of 7 days from creation.
...
- **THEN** the system SHALL return HTTP 201 with the invitation `id`, `invitedEmail`, `status` (`Pending`), and `expiresAt`
